                  ...
                  >
                    <div className="oa-designer__field-info">
                      <span className="oa-designer__field-label">
                        {field.label || '(未命名)'}
                        <small style={{ marginLeft: 6, color: '#999' }}>
                          [{widget?.label ?? field.type}]
                        </small>
                      </span>
                      {field.required ? <span className="oa-designer__field-required">必填</span> : null}
                    </div>
                    {/* 设计视图：用 widget 的 DesignView 渲染预览 */}
                    <div className="oa-designer__field-preview" style={{ pointerEvents: 'none' }}>
                      {widget ? <widget.DesignView field={field} selected={selectedId === field.id} /> : null}
                    </div>
